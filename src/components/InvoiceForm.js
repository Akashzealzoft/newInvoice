...
class InvoiceForm extends React.Component {
  ...
  handleCalculateTotal() {
    ...
    items = items.map((item) => ({
      ...item,
      total: parseFloat(item.price).toFixed(2) * parseInt(item.quantity),
    }));
    console.log(items);
    ...
    const totalAmount = items.reduce((total, project) => {
      const projectPrice = parseFloat(project.price);
      const projectQuantity = parseInt(project.quantity);

      return total + projectPrice * projectQuantity;
    }, 0);
  ...
  onItemizedItemEdit(evt) {
    ...
    var newItems = items.map(function (items) {
      //const newTotal = items.quantity * items.price;

      for (var key in items) {
        // const quantity =
        //   items["quantity"] && items.id == item.id ? items["quantity"] : 1;

        if (key == item.name && items.id == item.id) {
          items[key] = item.value;
        }
        // if (key == "price" && items.id == item.id) {
        //   items["total"] = item.value;
        // }
      }
      return items;
    });
  ...
  editField = (event) => {
    console.log(event, "Hello im the event");
    this.setState({
